...
export const LoginPage = () => {
  ...
  const { login, logout, checkingCredentials } = useAuthStore();
  ...
  const handleSubmit = async (event: any) => {
    ...
    if (result.ok) {
      login({ email, name: result.name , rol: result.rol });
      navigate('/dashboard')
    } else {
      logout(result.errorMessage);
              ...
              <form onSubmit={handleSubmit} className="flex flex-col gap-4 w-[95%] items-center">
                <div className="w-full flex flex-col gap-2 mt-[34px]">
                  <div className="flex gap-2">
                    ...
                    <label htmlFor="email"
                      className="font-bold text-[20px] text-white">Usuario</label>
                  </div>
                  <input onChange={(e) => setEmail(e.target.value)} type="text" id="email" placeholder="Nombre" value={email} className="h-[44px] rounded-xl w-full p-3" />
                </div>
                <div className="w-full flex flex-col gap-2 mt-[22px] relative">
                  ...
                  <input onChange={(e) => setPassword(e.target.value)} value={password} type="password" id="password" placeholder="************" className="h-12 rounded-xl w-full p-3" />
